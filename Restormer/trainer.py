...
class TrainerRestormer(Trainer):
    def __init__(self, params):
        super().__init__(params)
        self.model_params = self.params['model_params']
        self.input_shape = self.model_params['input_shape']
        self.batch_size = self.input_shape[0]
        self.model = Restormer(self.model_params)
        self.best_model = Restormer(self.model_params)
    ...
    def compute_losses(self, y_true, y_pred):
        losses_dict = {'L1_loss': tf.reduce_mean(tf.keras.losses.mae(y_true, y_pred))}
        total_loss = losses_dict['L1_loss']
